...
async function enrichSproutedRagi() {
  console.log('--- ENRICHING EXISTING SPROUTED RAGI FLOUR PRODUCT ---');

  // 1. Locate existing product in DB
  const existingProducts = await prisma.product.findMany({
    where: {
      OR: [
        { name: { contains: 'Sprouted Ragi', mode: 'insensitive' } },
        { name: { contains: 'Sprouted Raagi', mode: 'insensitive' } }
      ]
    },
    include: {
      categories: true,
  ...
  console.log(`Found ${existingProducts.length} matching product records in DB:`);
  existingProducts.forEach(p => {
    console.log(`- ID: ${p.id} | Name: "${p.name}" | SKU: ${p.sku} | Category: ${p.categories.map(c => c.name).join(', ')}`);
  });

  if (existingProducts.length === 0) {
    console.error('❌ No matching product found to enrich.');
    process.exit(1);
  }

  // Target primary product record (prefer Traditional Nutrition category or primary ID)
  const targetProduct = existingProducts.find(p => p.categories.some(c => c.name.toLowerCase().includes('nutrition') || c.name.toLowerCase().includes('traditional'))) || existingProducts[0];
  console.log(`\n🎯 TARGET PRODUCT FOR ENRICHMENT: ID = ${targetProduct.id} ("${targetProduct.name}")`);
  ...
  const contentSections = [
    ...
    },
    {
      ...
      title: 'Why Choose Suryodaya Farms Sprouted Ragi Flour?',
      content: {
        title: 'Why Choose Suryodaya Farms Sprouted Ragi Flour?',
        cards: [
          {
            icon: '🌱',
            title: 'Carefully Selected Finger Millet',
            description: 'Prepared from premium-quality finger millet that meets our strict quality standards.'
          },
          {
            icon: '💧',
            title: 'Naturally Sprouted',
            description: 'The grains are naturally sprouted under carefully controlled conditions before drying and milling.'
          },
          {
            icon: '🔬',
          ...
          },
          {
            ...
            description: 'Every stage—from grain selection to final packaging—is carried out under disciplined quality practices.'
          },
          {
            icon: '✨',
            title: 'Hygienically Processed',
            description: 'Processed with careful attention to cleanliness, food safety, and responsible handling.'
          },
          {
            icon: '🛍️',
    ...
    },
    {
      ...
      content: {
        title: 'Naturally Occurring Nutrients',
        items: [
          { name: 'Complex Carbohydrates', value: 'Sustained Energy' },
          { name: 'Dietary Fibre', value: 'Digestive Support' },
          { name: 'Plant-Based Protein', value: 'Muscle Building' },
          { name: 'Calcium', value: 'Bone & Teeth Health' },
          { name: 'Iron', value: 'Bio-available Iron' },
          { name: 'Naturally Occurring Vitamins', value: 'Essential Micronutrients' }
        ]
      },
      orderIndex: 4,
      isVisible: true
    },
    {
      ...
      content: {
        title: 'Ways to Enjoy',
        recipes: [
          { icon: '🥣', title: 'Porridge', description: 'Whisk with warm milk or water and jaggery for a wholesome breakfast porridge.' },
          { icon: '🥞', title: 'Dosa & Pancakes', description: 'Mix with rice batter or curd for crisp, nutritious sprouted ragi dosas.' },
          { icon: '🫓', title: 'Roti', description: 'Knead with warm water to make soft, gluten-free traditional flatbreads.' },
          { icon: '🥤', title: 'Malt Drinks & Smoothies', description: 'Blend into nutritious morning malt drinks and fitness smoothies.' },
          { icon: '🧁', title: 'Healthy Baking', description: 'Substitute partial wheat flour in cakes, cookies, and muffins for rich nutrition.' },
          { icon: '🍪', title: 'Homemade Snacks', description: 'Prepare traditional millet laddoos, murukku, and savory snacks.' }
        ]
      },
      orderIndex: 5,
      isVisible: true
    },
    {
      ...
      content: {
        title: 'Suggested Serving',
        items: [
          'Use according to your family\'s dietary preferences as part of a balanced and varied diet.',
          'If preparing foods for infants, young children, or individuals with specific dietary requirements, follow appropriate nutritional guidance and consult a qualified healthcare professional where needed.'
        ]
      },
      orderIndex: 6,
      isVisible: true
      ...
      content: {
        ...
        items: [
          ...
          },
          {
            ...
            answer: 'It can be used to prepare porridge, dosa, roti, malt drinks, pancakes, baked foods, smoothies, and a variety of traditional and modern recipes.'
          },
          {
            question: 'Is it suitable for everyday use?',
            answer: 'Yes. It can be enjoyed as part of a balanced and varied diet.'
          },
          {
    ...
    },
    {
      ...
      content: {
        title: 'Our Promise',
        html: '<p>Every pack of Suryodaya Farms Sprouted Ragi Flour reflects our dedication to quality, scientific responsibility, and customer trust.</p><p>We continuously improve our products because we believe every family deserves safe, wholesome, and consistently prepared nutrition.</p>'
      },
      orderIndex: 12,
      isVisible: true
    },
    {
      ...
      content: {
        title: 'The Suryodaya Difference',
        html: '<p>At Suryodaya Farms, we do more than simply mill grains. From carefully selecting premium finger millet to following a controlled natural sprouting process, maintaining hygienic processing practices, and using thoughtful packaging, every step is carried out with care and responsibility.</p><p>Our goal is to deliver a product that reflects the values we stand for—Nature, Science, Quality, and Trust—so you can enjoy the wholesome goodness of sprouted ragi with confidence, every day.</p>'
      },
      orderIndex: 13,
      isVisible: true
    }
  ...
  const updatedProduct = await prisma.product.update({
    ...
    data: {
      ...
      productType: 'Sprouted Millet Flour',
  ...
  console.log('\n✅ PRODUCT ENRICHED SUCCESSFULLY IN DATABASE!');
  ...
  console.log('\n--- ALL PRODUCT ENRICHMENT CHECKS PASSED PERFECTLY! ---');
}
